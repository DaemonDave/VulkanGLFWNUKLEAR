...
struct nk_vulkan_adapter
{
    ...
    VkSemaphore render_completed;  // internal semaphore if the render is complete
    VkSampler sampler;
    VkImage font_image;
...
static struct nk_glfw
{
    ...
    //! \struct nk_vulkan_adapter contains all VULKAN data structs
...
VkPipelineShaderStageCreateInfo create_shader(struct nk_vulkan_adapter *adapter, unsigned char *spv_shader, uint32_t size, VkShaderStageFlagBits stage_bit)
{
    ...
    VkResult result = vkCreateShaderModule(adapter->logical_device, &create_info, NULL, &module);
...
//! \fn create_descriptor_pool creates two pools for textures - uniform and sampler (image memories) 
void create_descriptor_pool(struct nk_vulkan_adapter *adapter)
{
    ...
    VkResult result = vkCreateDescriptorPool(adapter->logical_device, &pool_info, VK_NULL_HANDLE, &adapter->descriptor_pool);
    NK_ASSERT(result == VK_SUCCESS);
}
//! \fn create_uniform_descriptor_set_layout creates one descriptorsetlayout for textures uniform for descriptorsetlayout creation struct 
void create_uniform_descriptor_set_layout(struct nk_vulkan_adapter *adapter)
{
    ...
    VkResult result = vkCreateDescriptorSetLayout( adapter->logical_device, &descriptor_set_info, NULL, &adapter->uniform_descriptor_set_layout);

    NK_ASSERT(result == VK_SUCCESS);
}
//! \fn create_texture_descriptor_set_layout creates one descriptorsetlayoutbinding for one descriptorsetlayoutcreateinfo
void create_texture_descriptor_set_layout(struct nk_vulkan_adapter *adapter)
{
    ...
    VkResult result = vkCreateDescriptorSetLayout( adapter->logical_device, &descriptor_set_info, NULL, &adapter->texture_descriptor_set_layout);

    NK_ASSERT(result == VK_SUCCESS);
}
//! \fn create_and_update_uniform_descriptor_set creates one descriptorsetallocator for uniform buffer (4x4 Matrix) created into WriteDescriptorSet
void create_and_update_uniform_descriptor_set( struct nk_vulkan_adapter *adapter)
{
    ...
    VkResult result = vkAllocateDescriptorSets(adapter->logical_device, &allocate_info, &adapter->uniform_descriptor_set);
    NK_ASSERT(result == VK_SUCCESS);
    ...
    vkUpdateDescriptorSets(adapter->logical_device, 1, &descriptor_write, 0, VK_NULL_HANDLE);
}
//! \fn create_texture_descriptor_sets creates NK_GLFW_MAX_TEXTURES descriptorsetlayouts for NK_GLFW_MAX_TEXTURES descriptorsetlayoutcreateinfo
void create_texture_descriptor_sets(struct nk_vulkan_adapter *adapter)
{
    VkDescriptorSetLayout *descriptor_set_layouts = (VkDescriptorSetLayout *)malloc(NK_GLFW_MAX_TEXTURES * sizeof(VkDescriptorSetLayout));
    VkDescriptorSet *descriptor_sets = (VkDescriptorSet *)malloc(NK_GLFW_MAX_TEXTURES * sizeof(VkDescriptorSet));

    adapter->texture_descriptor_sets = (struct nk_vulkan_texture_descriptor_set *)malloc( NK_GLFW_MAX_TEXTURES * sizeof(struct nk_vulkan_texture_descriptor_set));
    ...
    VkResult result = vkAllocateDescriptorSets(adapter->logical_device, &allocate_info, descriptor_sets);
    NK_ASSERT(result == VK_SUCCESS);
	// aim at every image allocated
    for (i = 0; i < NK_GLFW_MAX_TEXTURES; i++)
    {
...
void update_texture_descriptor_set( struct nk_vulkan_adapter *adapter, struct nk_vulkan_texture_descriptor_set *texture_descriptor_set, VkImageView image_view)
{
...
//! \fn create_pipeline_layout creates uniform and texture descriptorsetlayouts for 2 layouts created into pipelinelayout
void create_pipeline_layout(struct nk_vulkan_adapter *adapter)
{
    ...
    VkResult result = (vkCreatePipelineLayout(adapter->logical_device, &pipeline_layout_info, NULL, &adapter->pipeline_layout));
    NK_ASSERT(result == VK_SUCCESS);
}
//! \fn prepare_pipeline creates triangle vertex topology  pipelineassemblystate and basic rasterization, two shaders (vertex and fragment) one viewpoint, one color blend, one pipeline and then destroys shaders.
...
//! \fn prepare_render_pass create one color attachment and one subpass into one renderpass
void prepare_render_pass(struct nk_vulkan_adapter *adapter)
{
    ...
    VkResult result = vkCreateRenderPass( adapter->logical_device, &render_pass_info, NULL, &adapter->render_pass);
    NK_ASSERT(result == VK_SUCCESS);
}
//! \fn prepare_framebuffers creates per-imageviews framebuffers and assigns framebuffers as the same size as imageviews
void prepare_framebuffers(struct nk_vulkan_adapter *adapter)
{
    adapter->framebuffers = (VkFramebuffer *)malloc(adapter->image_views_len * sizeof(VkFramebuffer));
    ...
    for (i = 0; i < adapter->image_views_len; i++)
    {
        ...
        VkResult result = (vkCreateFramebuffer(adapter->logical_device, &framebufferCreateInfo, NULL, &adapter->framebuffers[i]));
        NK_ASSERT(result == VK_SUCCESS);
    }
...
void prepare_command_buffers(struct nk_vulkan_adapter *adapter)
{
    ...
    VkResult result = vkAllocateCommandBuffers( adapter->logical_device, &allocate_info, adapter->command_buffers);
    NK_ASSERT(result == VK_SUCCESS);
    adapter->command_buffers_len = 1;
}
//! \fn prepare_semaphores prepares 1 semaphore for the render complete condition
void prepare_semaphores(struct nk_vulkan_adapter *adapter)
{
    ...
    // CONFIGURE AT THE RENDER_COMPLETE 
    VkResult result = (vkCreateSemaphore(adapter->logical_device, &semaphore_info, VK_NULL_HANDLE, &adapter->render_completed));
    NK_ASSERT(result == VK_SUCCESS);
}
...
void create_buffer_and_memory(struct nk_vulkan_adapter *adapter, VkBuffer *buffer, VkBufferUsageFlags usage, VkDeviceMemory *memory, VkDeviceSize size)
{
    ...
    VkResult result;
    result = vkCreateBuffer(adapter->logical_device, &buffer_info, VK_NULL_HANDLE, buffer);
    NK_ASSERT(result == VK_SUCCESS);
    ...
    result = vkAllocateMemory(adapter->logical_device, &alloc_info, VK_NULL_HANDLE, memory);
    NK_ASSERT(result == VK_SUCCESS);
    result = vkBindBufferMemory(adapter->logical_device, *buffer, *memory, 0);
    NK_ASSERT(result == VK_SUCCESS);
}
//! \fn create_render_resources constructs the VULKAN pipeline from components.
...
void cleanup_render_resources(struct nk_vulkan_adapter *adapter)
{
    ...
    vkDestroyDescriptorSetLayout(adapter->logical_device, adapter->texture_descriptor_set_layout, VK_NULL_HANDLE);
    vkDestroyDescriptorSetLayout(adapter->logical_device, adapter->uniform_descriptor_set_layout, VK_NULL_HANDLE);
    ...
    {
        vkDestroyFramebuffer(adapter->logical_device, adapter->framebuffers[i], VK_NULL_HANDLE);
    }
...
void create_command_pool(struct nk_vulkan_adapter *adapter, uint32_t graphics_queue_family_index)
{
    ...
    VkResult result =
        (vkCreateCommandPool(adapter->logical_device, &pool_info, VK_NULL_HANDLE, &adapter->command_pool));
    NK_ASSERT(result == VK_SUCCESS);
}
//! \fn nk_glfw3_device_create - the constructor function that sets up the gui overhead
NK_API void nk_glfw3_device_create(VkDevice logical_device,
                                   VkPhysicalDevice physical_device,
                                   uint32_t graphics_queue_family_index,
                                   VkImageView *image_views,
                                   uint32_t image_views_len, uint32_t width,
                                   uint32_t height, VkFormat color_format)
{	// SETS UP THE DATA VALUES FOR CONSTRUCTION
    ...
    adapter->framebuffers_len = 0;
    adapter->command_buffers = (VkCommandBuffer *)malloc(image_views_len * sizeof(VkCommandBuffer));
	// prepare the render complete semaphores
    prepare_semaphores(adapter);
	// create vertex data memory
    ...
    // create uniform (push constants) memory
...
NK_API
VkSemaphore nk_glfw3_render(enum nk_anti_aliasing AA, VkQueue graphics_queue, uint32_t buffer_index, VkSemaphore wait_semaphore)
{
    struct nk_vulkan_adapter *adapter = &glfw.adapter;
                            ...
                            &adapter->uniform_descriptor_set, 0, VK_NULL_HANDLE);
    {
        ...
        nk_draw_foreach(cmd, &glfw.ctx, &adapter->cmds)
        {
            ...
            if (cmd->texture.ptr && cmd->texture.ptr != current_texture)
            {
                ...
                if (!found)
                {
                    update_texture_descriptor_set(adapter, &adapter->texture_descriptor_sets[i], (VkImageView)cmd->texture.ptr);
...
{
    struct nk_vulkan_adapter *adapter = &glfw.adapter;
    vkFreeCommandBuffers(adapter->logical_device, adapter->command_pool, adapter->command_buffers_len, adapter->command_buffers);
