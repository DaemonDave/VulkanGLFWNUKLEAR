...
//! \include AlgSelector as part of window - uses customized widgets for algorithm selection
//#include "algorithm-selector.c"
//#include "overview.c"

// these are the converts from NVIDIA 
//#include "nvpro.c"
...
{
  struct nk_colorf background = {settings->bg_color[0], settings->bg_color[1], settings->bg_color[2], settings->bg_color[3]};
  /* Input */
  glfwPollEvents();
  nk_glfw3_new_frame();
  /* GUI */
	       ...
	       NK_WINDOW_MINIMIZABLE | NK_WINDOW_TITLE))
  {
    ...
    nk_layout_row_dynamic(ctx, 25, 1);
    float zoomf = 1.0;
    nk_property_float(ctx, "Zoomf:", 0.0, &zoomf, 100.0, 10.0, 1);
  ...
  nk_end(ctx);
  // the tie-in for new menu's and the new stuff
  /* -------------- EXAMPLES ---------------- */
  /*calculator(ctx);*/
  //overview(ctx);
  //AlgSelector(ctx);
  ...
  /// DRE 2024 - wait semaphore is never used it's NULL
  return nk_glfw3_render(NK_ANTI_ALIASING_ON, graphics_queue, buffer_index, wait_semaphore);
}
