...
//		} VkPresentInfoKHR;
    ...
    bool checkDeviceExtensionSupport(VkPhysicalDevice device)
    {
        ...
        // creates an extension string array below
        std::vector<VkExtensionProperties> availableExtensions(extensionCount);
        // available device extensions here...
        vkEnumerateDeviceExtensionProperties(device, nullptr, &extensionCount, availableExtensions.data());

        std::set<std::string> requiredExtensions(deviceExtensions.begin(), deviceExtensions.end());
        std::cerr << "\n Instance Extensions originally wanted:\n" << extensionCount << std::endl;
        for (unsigned int i = 0; i < extensionCount; i++)
        {
            //std::cerr << "<" << i << availableExtensions[i].data << "> " << std::endl;
            std::cerr << "<" << i << std::endl;
    ...
    bool checkValidationLayerSupport()
    {
        ...
        for (const char *layerName : validationLayers)
        {
            ...
            for (const auto &layerProperties : availableLayers)
            {
                ...
                {
                    std::cerr << "<" << layerName << "> [" << layerProperties.layerName << "] " << std::endl;
                    layerFound = true;
                    break;
                }
            }
